...
export const runDeploy = async (selectedSite) => {
  try {
    const data = await axios(
      `/${pluginId}/deploy/run?site=${selectedSite.displayName}`,
      { method: "GET" }
    );
    return data.data;
  } catch (error) {
...
export const getDeployments = async (selectedSite) => {
  try {
    const response = await axios(
      `/${pluginId}/deploy/list?site=${selectedSite.displayName}`,
      { method: "GET" }
    );
    return response.data;
  } catch (error) {
...
export const deployAvailability = async (selectedSite) => {
  try {
    const response = await axios(
      `/${pluginId}/deploy/availability?site=${selectedSite.displayName}`,
      {
        method: "GET",
      }
    );
